...
def add_static_cache_headers(response):
    """Cache immutable-ish static assets without caching dynamic page responses."""
    if request.path.startswith("/static/"):
        ...
        elif path.endswith((".png", ".jpg", ".jpeg", ".webp", ".gif", ".svg", ".ico", ".mp4", ".webm")):
            response.cache_control.public = True
...
def init_db():
    ...
    try:
        ...
        """)
        # Query indexes for the most common filters/joins. They are safe to
        # create on existing deployments because IF NOT EXISTS is used.
        for index_sql in (
            "CREATE INDEX IF NOT EXISTS idx_users_role_name ON users(role, name)",
            "CREATE INDEX IF NOT EXISTS idx_users_department_year ON users(department, year)",
            "CREATE INDEX IF NOT EXISTS idx_skills_user_type_name ON skills(user_id, skill_type, skill_name)",
            "CREATE INDEX IF NOT EXISTS idx_skills_type_name ON skills(skill_type, skill_name)",
            "CREATE INDEX IF NOT EXISTS idx_ratings_receiver ON ratings(receiver_id)",
            "CREATE INDEX IF NOT EXISTS idx_requests_receiver_status ON requests(receiver_id, status)",
            "CREATE INDEX IF NOT EXISTS idx_requests_sender_receiver_status ON requests(sender_id, receiver_id, status)",
...
def get_profiles_by_role(role, exclude_user_id=None, limit=6):
    """Fetch profiles with skills/ratings in three batched queries instead of 2N+1."""
    db = get_db()
    rows = db.execute(
    ...
    skills = db.execute(
        f"SELECT * FROM skills WHERE user_id IN ({placeholders}) AND skill_type='offered' ORDER BY skill_name",
        user_ids
    ).fetchall()
    rating_rows = db.execute(
        f"SELECT receiver_id, AVG(stars) AS avg_stars, COUNT(*) AS total FROM ratings "
        f"WHERE receiver_id IN ({placeholders}) GROUP BY receiver_id",
        user_ids
    ).fetchall()

    offered_by_user = {uid: [] for uid in user_ids}
    for skill in skills:
        offered_by_user[skill["user_id"]].append(skill)
    ratings_by_user = {
        row["receiver_id"]: (round(row["avg_stars"], 1) if row["avg_stars"] else 0, row["total"])
        for row in rating_rows
    }

    return [{
        "user": u,
        "offered": offered_by_user[u["id"]],
        "avg_rating": ratings_by_user.get(u["id"], (0, 0))[0],
        "total_ratings": ratings_by_user.get(u["id"], (0, 0))[1],
    } for u in rows]


def compute_matches(user_id):
    """Compute skill matches using batched DB reads (constant query count)."""
    db = get_db()

    mine = db.execute(
        "SELECT skill_type, skill_name FROM skills WHERE user_id=?", (user_id,)
    ).fetchall()
    my_offered = {r["skill_name"].strip().lower() for r in mine if r["skill_type"] == "offered"}
    my_wanted = {r["skill_name"].strip().lower() for r in mine if r["skill_type"] == "wanted"}

    all_skills = db.execute(
        "SELECT user_id, skill_type, skill_name FROM skills WHERE user_id != ?", (user_id,)
    ).fetchall()
    users_data = {}
    for row in all_skills:
        bucket = users_data.setdefault(row["user_id"], {"offered": set(), "wanted": set()})
        bucket[row["skill_type"]].add(row["skill_name"].strip().lower())

    other_ids = list(users_data)
    if not other_ids:
        return [], []

    placeholders = ",".join(["?"] * len(other_ids))
    user_rows = db.execute(
        f"SELECT * FROM users WHERE id IN ({placeholders})", other_ids
    ).fetchall()
    ...
    rating_by_id = {
        r["receiver_id"]: (round(r["avg_stars"], 1) if r["avg_stars"] else 0, r["total"])
        for r in rating_rows
    }

    perfect, partial = [], []
    for other_id, buckets in users_data.items():
        they_teach_me, _ = fuzzy_intersect(my_wanted, buckets["offered"])
        i_teach_them, _ = fuzzy_intersect(my_offered, buckets["wanted"])
        if not they_teach_me or other_id not in user_by_id:
            continue

        avg_rating, total_ratings = rating_by_id.get(other_id, (0, 0))
        entry = {
            "user": user_by_id[other_id],
            "they_teach_me": sorted(they_teach_me),
            "i_teach_them": sorted(i_teach_them),
            "avg_rating": avg_rating,
            "total_ratings": total_ratings,
        }
        (perfect if i_teach_them else partial).append(entry)
...
def get_conversation_list(uid):
    ...
    dms = db.execute(
        """SELECT c.id, c.created_at,
                  ...
                  (SELECT content FROM messages m WHERE m.conversation_id=c.id
                   AND m.deleted=0
                   AND instr(COALESCE(m.hidden_for, '[]'), '"' || ? || '"')=0
    ...
    # Hide conversations that have no messages visible to this user.
    # Uses the existing messages fields only; no schema changes.
    dms = [c for c in dms if c["last_msg"] is not None]
    groups = [c for c in groups if c["last_msg"] is not None]
...
def clear_chat(conv_id):
    """Clear a conversation for the current user without deleting database rows."""
    uid = session["user_id"]
    db = get_db()
    conv = db.execute("SELECT * FROM conversations WHERE id=?", (conv_id,)).fetchone()
    if not conv or not _user_can_access_conv(db, conv, uid):
    ...
    rows = db.execute(
        "SELECT id, hidden_for FROM messages WHERE conversation_id=?",
        (conv_id,)
    ).fetchall()

    for row in rows:
        try:
            hidden_for = json.loads(row["hidden_for"] or "[]")
            if not isinstance(hidden_for, list):
                hidden_for = []
        except Exception:
            hidden_for = []
        if uid not in hidden_for:
            ...
            db.execute(
                "UPDATE messages SET hidden_for=? WHERE id=?",
                (json.dumps(hidden_for), row["id"])
            )
