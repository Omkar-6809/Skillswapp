...
=============================================
A Flask + SQLite web app that lets students offer skills they know and
...
# Database helpers
# -----------------------------------------------------------------------
# Local development uses SQLite. On Vercel, set DATABASE_URL to a hosted
# PostgreSQL connection string (Neon, Supabase, etc.). This keeps the same
# application code working in both environments.
DATABASE = os.environ.get("DATABASE_PATH", os.path.join(BASE_DIR, "database.db"))
DATABASE_URL = os.environ.get("DATABASE_URL", "").strip()


class DBConnection:
    """Small compatibility wrapper so the existing app can use SQLite or
    PostgreSQL without rewriting every SQL query in the application."""
...
class PGCursor:
    ...
    def __iter__(self):
        # Keep compatibility with the original SQLite code, which
        # iterates directly over the result of db.execute(...).
...
def get_db():
    """Return one database connection per Flask request."""
    ...
    if db is None:
        if DATABASE_URL:
            ...
            db = g._database = DBConnection(conn, postgres=True)
        else:
            conn = sqlite3.connect(DATABASE)
            conn.row_factory = sqlite3.Row
            conn.execute("PRAGMA foreign_keys = ON")
            db = g._database = DBConnection(conn, postgres=False)
    return db
...
def init_db():
    """Create the complete schema when using a fresh database.
    Existing SQLite databases are left untouched so current users remain safe.
    """
    if DATABASE_URL:
        if psycopg is None:
    ...
    # Existing SQLite initialization is retained for local/PythonAnywhere use.
    conn = sqlite3.connect(DATABASE)
    conn.execute("PRAGMA foreign_keys = ON")
    cur = conn.cursor()
    cur.execute("""
        CREATE TABLE IF NOT EXISTS users (
            ...
            password TEXT NOT NULL, department TEXT, year TEXT, bio TEXT, profile_picture TEXT,
            role TEXT DEFAULT 'student' CHECK(role IN ('student','faculty')),
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        )
    """)
    cur.execute("""CREATE TABLE IF NOT EXISTS skills (
        id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, skill_name TEXT NOT NULL,
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS requests (
        ...
        skill_offered TEXT, skill_wanted TEXT, status TEXT NOT NULL DEFAULT 'pending'
        CHECK(status IN ('pending','accepted','rejected','completed')), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY(sender_id) REFERENCES users(id) ON DELETE CASCADE,
        FOREIGN KEY(receiver_id) REFERENCES users(id) ON DELETE CASCADE)""")
    cur.execute("""CREATE TABLE IF NOT EXISTS ratings (
        id INTEGER PRIMARY KEY AUTOINCREMENT, giver_id INTEGER NOT NULL, receiver_id INTEGER NOT NULL,
        request_id INTEGER, stars INTEGER NOT NULL CHECK(stars BETWEEN 1 AND 5), feedback TEXT,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS teams (
        ...
        team_type TEXT NOT NULL DEFAULT 'other' CHECK(team_type IN ('hackathon','coding_competition','project','other')),
        creator_id INTEGER NOT NULL, max_members INTEGER DEFAULT 4, event_date TEXT,
        status TEXT DEFAULT 'open' CHECK(status IN ('open','closed')), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS team_members (
        ...
        role TEXT DEFAULT 'member' CHECK(role IN ('leader','member')), status TEXT DEFAULT 'pending'
        CHECK(status IN ('pending','accepted','rejected')), joined_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY(team_id) REFERENCES teams(id) ON DELETE CASCADE,
        FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE, UNIQUE(team_id,user_id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS conversations (
        id INTEGER PRIMARY KEY AUTOINCREMENT, user1_id INTEGER, user2_id INTEGER, team_id INTEGER,
        is_group INTEGER NOT NULL DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY(user1_id) REFERENCES users(id) ON DELETE CASCADE,
        FOREIGN KEY(user2_id) REFERENCES users(id) ON DELETE CASCADE,
        FOREIGN KEY(team_id) REFERENCES teams(id) ON DELETE CASCADE, UNIQUE(user1_id,user2_id), UNIQUE(team_id))""")
    cur.execute("""CREATE TABLE IF NOT EXISTS messages (
        id INTEGER PRIMARY KEY AUTOINCREMENT, conversation_id INTEGER NOT NULL, sender_id INTEGER NOT NULL,
        content TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, deleted INTEGER DEFAULT 0,
        edited INTEGER DEFAULT 0, reactions TEXT DEFAULT '{}', reply_to INTEGER, hidden_for TEXT DEFAULT '[]',
        FOREIGN KEY(conversation_id) REFERENCES conversations(id) ON DELETE CASCADE,
        FOREIGN KEY(sender_id) REFERENCES users(id) ON DELETE CASCADE)""")
    cur.execute("""CREATE TABLE IF NOT EXISTS message_reports (
        id INTEGER PRIMARY KEY AUTOINCREMENT, message_id INTEGER NOT NULL, reporter_id INTEGER NOT NULL,
        reason TEXT, details TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY(message_id) REFERENCES messages(id) ON DELETE CASCADE,
        FOREIGN KEY(reporter_id) REFERENCES users(id) ON DELETE CASCADE)""")
    for col_sql in [
        "ALTER TABLE users ADD COLUMN designation TEXT", "ALTER TABLE users ADD COLUMN qualifications TEXT",
        "ALTER TABLE messages ADD COLUMN deleted INTEGER DEFAULT 0", "ALTER TABLE messages ADD COLUMN edited INTEGER DEFAULT 0",
        "ALTER TABLE messages ADD COLUMN reactions TEXT DEFAULT '{}'", "ALTER TABLE messages ADD COLUMN reply_to INTEGER",
        "ALTER TABLE messages ADD COLUMN hidden_for TEXT DEFAULT '[]'"
    ]:
